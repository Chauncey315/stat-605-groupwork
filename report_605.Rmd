---
title: "Final Report on IMDb Data"
...
```{r, echo=FALSE}
top_5 <- read.table("Result/movie2018.txt", sep=",", header = TRUE)
...
1. Whether the numbers of voting can represent well interests of  the audience. The numbers of voting represent that the audience has watched the production and rated it, so it is sentive to time and other variables. For example, for a just released movie, people have less chance to access it than the old movie, and perhaps they just go to cinema but not watch online. So it is possible that a moive of large numbers of voting is not because of more interests from the audience but more ways to access it.
2. There are too many latent variables to influence whether a cast or production is good, and they also might make an influence on each other. So we need a more data and further research to get conclusion who is best or lucky cast and production.
...
3. Do the regression and test on related features, and find more significant features to get a more powerful interpretation on our question. 
4. To find the trend of changes in TOP cast and production, deep into theirs features and find what features create a good cast or production.  

## Conclusion
From our results, we can make these conclusion as followed.
1. For most categories of production, ratings and audience interests don’t change much over time. 
2. Movie is always dominated among the audience but most movies in the market do not have a higher reviews.
3. Production based TV is more and more popolar in recent year, especially in TV Episode and Series.
