...
The goal of the project is to get insights into the [IMDb data](https://www.imdb.com/interfaces/). We mainly focus on three questions:  
...
To answer these questions, we parallelize our computation in 850 jobs on the CHTC platform, plot the related pictures and make a statistical description. In general, we find that there are no significant changes in all kinds of productions from 1900 to 2019. And Movie and TV series become more and more fashionable, which has called more attention of the audience in recent 30 years. Specially, we list top5 best cast in each role and top5 productions in each category.
...
To answer questions 1&2, we select ratings variable and voting variable as the criterion of audience interests and parallelize to calculate average ratings and voting of each production(movie, short, tvShort,...) for each year on the CHTC. Then, we plot the trend of average ratings and voting over time.

To answer question 3, we process $merge\_br.csv$ file, and parallelize to sort and get 5 productions with the highest ratings in each category and output to $best.csv$. For more accurate and significant, we filter out the production with less than or equal to 800 reviews. To find the best cast, we select the data related to movie, and parallelize 5 jobs to process $actor.csv$, $actress.csv$, $director.csv$, $producer.csv$, $composer.csv$. Specifically, we calculate average ratings of all works for each cast, and sort the Top 10 person in each category. And we also get all works of top 3 one of these 10 person in each category.
...
From the plot of average ratings of traditional category over year, we can find that there is no significant changes for average ratings in each category over year, even if video and video games have a noticeable fluctuation in around 1960. Among the difference category, in general, the average ratings of video and video games is slightly higher than that of movie of short. But in the recent 10 years, the average ratings of short have a significant increase

![average votings of traditional category over year](votings_productions.png)

For the audience interest part, we can find the average numbers of votes in movie is significantly larger than that of other categories, especially in recent 20 years. From 1980-2019, there is a bell-shaped curve on the movie category. Specifically, the number of votes grows to the peak in 2000 and gradually decline. For other category, there is no noticeable changes.

![average ratings of category based TV over year](ratings_tv.png)  

On the other hand, in category related to TV, there is also no significant changes for average ratings in each category over year, even if TV movie and series have a noticeable fluctuation in around 1940. In general, TV episode and mini series have higher ratings than others.
